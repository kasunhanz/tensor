...
func InventoryWrite(user models.User, inventory models.Inventory) bool {
	...
	// if object has granted team get those teams to list
	for _, v := range inventory.Roles {
		if v.Type == "team" && (v.Role == INVENTORY_ADMIN || v.Role == INVENTORY_UPDATE) {
			teams = append(teams, v.TeamID)
		}

		if v.Type == "user" && v.UserID == user.ID && (v.Role == INVENTORY_ADMIN || v.Role == INVENTORY_UPDATE) {
			return true
		}
	}
...
func InventoryUse(user models.User, inventory models.Inventory) bool {
	...
	// if object has granted team get those teams to list
	for _, v := range inventory.Roles {
		if v.Type == "team" && (v.Role == INVENTORY_ADMIN || v.Role == INVENTORY_USE) {
			teams = append(teams, v.TeamID)
		}

		if v.Type == "user" && v.UserID == user.ID && (v.Role == INVENTORY_ADMIN || v.Role == INVENTORY_USE) {
			return true
		}
	}
...
func InventoryAddHoc(user models.User, inventory models.Inventory) bool {
	...
	for _, v := range inventory.Roles {
		if v.Type == "team" &&
			(v.Role == INVENTORY_ADMIN || v.Role == INVENTORY_ADD_HOC) {
			teams = append(teams, v.TeamID)
		}
