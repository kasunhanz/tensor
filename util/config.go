...
func init() {
	...
	flag.BoolVar(&Upgrade, "upgrade", false, "upgrade hilbertspace")
...
func (conf *configType) Scan() {
	...
	fmt.Print(" > DB Name (default hilbertspace): ")
	fmt.Scanln(&conf.MySQL.DbName)
	if len(conf.MySQL.DbName) == 0 {
		conf.MySQL.DbName = "hilbertspace"
	}
